...
public:
  ...
  bool applyGamma;
  Model(string const &path, bool gamma = false) : applyGamma(gamma) {
    // load my mesh.
  }
...
//     // model data
//     vector<Texture> textures_loaded;	// stores all the textures loaded so
//     far, optimization to make sure textures aren't loaded more than once.
//     vector<Mesh>    meshes;
//     string directory;
...
//     the resulting meshes in the meshes vector. void loadModel(string const
//     &path)
//     {
...
//     mesh located at the node and repeats this process on its children nodes
//     (if any). void processNode(aiNode *node, const aiScene *scene)
//     {
...
//             organized (like relations between nodes). aiMesh* mesh =
//             scene->mMeshes[node->mMeshes[i]];
//             meshes.push_back(processMesh(mesh, scene));
//         }
//         // after we've processed all of the meshes (if any) we then
//         recursively process each of the children nodes for(unsigned int i =
...
//             uses its own vector class that doesn't directly convert to glm's
//             vec3 class so we transfer the data to this placeholder glm::vec3
//             first.
//             // positions
...
//                 coordinates so we always take the first set (0). vec.x =
//                 mesh->mTextureCoords[0][i].x; vec.y =
//                 mesh->mTextureCoords[0][i].y; vertex.TexCoords = vec;
//                 // tangent
...
//         triangle) and retrieve the corresponding vertex indices. for(unsigned
//         int i = 0; i < mesh->mNumFaces; i++)
//         {
...
//         diffuse texture should be named
//         // as 'texture_diffuseN' where N is a sequential number ranging from
//         1 to MAX_SAMPLER_NUMBER.
...
//             iteration: skip loading a new texture bool skip = false;
...
//                 textures_loaded.push_back(texture);  // store it as texture
//                 loaded for entire model, to ensure we won't unnecesery load
//                 duplicate textures.
//             }
//         }
//         return textures;
